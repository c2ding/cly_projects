...
/**
  ...
  *           Value：值
  */
static void I2C_MPU6050_WriteData(uint16_t Addr, uint8_t Reg, uint8_t Value)
{
	HAL_StatusTypeDef status = HAL_OK;
  
	status = HAL_I2C_Mem_Write(&i2c2, Addr, (uint16_t)Reg, I2C_MEMADD_SIZE_8BIT, &Value, 1, 3000);
...
HAL_StatusTypeDef I2C_MPU6050_WriteBuffer(uint16_t Addr, uint8_t Reg, uint16_t RegSize, uint8_t *pBuffer, uint16_t Length)
{
	...
	status = HAL_I2C_Mem_Write(&i2c2, Addr, (uint16_t)Reg, RegSize, pBuffer, Length, 3000); 
...
uint8_t I2C_MPU6050_ReadData(uint16_t Addr, uint8_t Reg)
{
	...
	status = HAL_I2C_Mem_Read(&i2c2, Addr, Reg, I2C_MEMADD_SIZE_8BIT, &value, 1, 3000);
	/* 检测I2C通信状态 */
	if(status != HAL_OK)
	{
		/* 调用I2C通信错误处理函数 */
...
/**
  ...
  * 返 回 值: HAL_StatusTypeDef：操作结果
  */
static HAL_StatusTypeDef I2C_MPU6050_ReadBuffer(uint16_t Addr, uint8_t Reg, uint16_t RegSize, uint8_t *pBuffer, uint16_t Length)
{
	HAL_StatusTypeDef status = HAL_OK;

	status = HAL_I2C_Mem_Read(&i2c2, Addr, (uint16_t)Reg, RegSize, pBuffer, Length, 3000);
...
{ 
	return (HAL_I2C_IsDeviceReady(&i2c2, DevAddress, Trials, 3000));
}
/*----------------------------------------------------------------------------*/
/**
  * 函数功能: 写数据到MPU6050寄存器
...
/**
  * 函数功能: 读取MPU6050的ID
  */ 
bool MPU6050ReadID(void)
{
	unsigned char Re = 0;
    MPU6050_ReadData(MPU6050_RA_WHO_AM_I, &Re, 1); //读器件地址
	if(Re != 0x68) {return true;} else {return false;}
}
/*----------------------------------------------------------------------------*/
/**
