/*
#include "stm32f1xx_oled12864_spi_3line.h" //0.96OLED12864 驱动
...
#endif
//数据、命令
#define OLED12864_DC_PIN	GPIO_PIN_12
#define OLED12864_DC_PORT	GPIOB
#define OLED12864_DC_H()	HAL_GPIO_WritePin(OLED12864_DC_PORT, OLED12864_DC_PIN, GPIO_PIN_SET)
#define OLED12864_DC_L()	HAL_GPIO_WritePin(OLED12864_DC_PORT, OLED12864_DC_PIN, GPIO_PIN_RESET)
//片选
#if defined(__OLED12864_CS__)
#define OLED12864_CS_PIN	GPIO_PIN_10
#define OLED12864_CS_PORT	GPIOB
#define OLED12864_CS_H()	HAL_GPIO_WritePin(OLED12864_CS_PORT, OLED12864_CS_PIN, GPIO_PIN_SET)
#define OLED12864_CS_L()	HAL_GPIO_WritePin(OLED12864_CS_PORT, OLED12864_CS_PIN, GPIO_PIN_RESET)
#endif
//使用的SPI端口
#if defined(__STM32F1XX_SPI1_H__)
#define SSD1315_SPI hspi1
#endif
#if defined(__STM32F1XX_SPI2_H__)
#define SSD1315_SPI hspi2
#endif
#if defined(__STM32F1XX_SPI3_H__)
#define SSD1315_SPI hspi3
#endif
/* 私有类型定义 --------------------------------------------------------------*/
/* 私有变量 ------------------------------------------------------------------*/
...
void ssd1315_init(void)
{
	...
	GPIO_InitStruct.Pin = OLED12864_DC_PIN;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	HAL_GPIO_Init(OLED12864_DC_PORT, &GPIO_InitStruct);
	HAL_GPIO_LockPin(OLED12864_DC_PORT, OLED12864_DC_PIN); //锁定配置
	OLED12864_DC_L();
#if defined(__OLED12864_CS__)
	GPIO_InitStruct.Pin = OLED12864_CS_PIN;
	//GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	//GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	HAL_GPIO_Init(OLED12864_CS_PORT, &GPIO_InitStruct);
	HAL_GPIO_LockPin(OLED12864_CS_PORT, OLED12864_CS_PIN); //锁定配置
	OLED12864_CS_L();
#endif
#if defined(__OLED12864_RES__)
	...
	OLED12864_RES_H();
#endif
	status |= HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t*)&SSD1315_COMMAND, sizeof(SSD1315_COMMAND), 1000);
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
#endif
...
static void ssd1315_yx(uint8_t p_y, uint8_t p_x)
{
	...
	s_tx[2] = p_x & 0x0F;
	HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t*)&s_tx, 3, 1000);
	OLED12864_DC_H();
}
/*----------------------------------------------------------------------------*/
//清屏
void ssd1315_clear(void)
{
	...
	{
		ssd1315_yx(s_y, 0);
		HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&s_tx, sizeof(s_tx), 1000);
	}
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
#endif
...
#endif
	ssd1315_yx(p_y, p_x);
	HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&asciitab_dzk_5x7[p_datum-32][0], 6, 1000);
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
#endif
}
...
void ssd1315_write_string_5x7(uint8_t p_y, uint8_t p_x, uint8_t *p_datum, uint8_t p_length)
{
	...
	do {
		s_datum = (*p_datum++)-32;
		HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&asciitab_dzk_5x7[s_datum][0], 6, 1000);
	} while (--p_length);
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
#endif
...
void ssd1315_write_string_8x16(uint8_t p_y, uint8_t p_x, uint8_t *p_datum, uint8_t p_length)
{
	...
	do {
		s_datum = (*p_datum++)-32;
		ssd1315_yx(p_y, p_x);
		HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&asciitab_dzk_8x16[s_datum][0], 8, 1000);
		ssd1315_yx(p_y+1, p_x);
		HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&asciitab_dzk_8x16[s_datum][8], 8, 1000);
		p_x += 8;
	} while (--p_length);
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
...
/*----------------------------------------------------------------------------*/
//按指定位置显示一个8x16字符
void ssd1315_write_word_8x16(uint8_t p_y, uint8_t p_x, uint8_t p_datum)
{
#if defined(__OLED12864_CS__)
	OLED12864_CS_L();
#endif
	ssd1315_yx(p_y, p_x);
	HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&asciitab_dzk_8x16[p_datum-32][0], 8, 1000);
	ssd1315_yx(p_y+1, p_x);
	HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&asciitab_dzk_8x16[p_datum-32][8], 8, 1000);
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
#endif
}
/*----------------------------------------------------------------------------*/
void ssd1315_write_word(uint8_t p_y, uint8_t p_x, uint8_t p_datum) //按指定位置显示一个16*16字符(汉字)
{
#if defined(__OLED12864_CS__)
	OLED12864_CS_L();
#endif
	ssd1315_yx(p_y, p_x);
	HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&chinese_characters[p_datum][0], 16, 1000);

	ssd1315_yx(p_y+1, p_x);
	HAL_SPI_Transmit(&SSD1315_SPI, (uint8_t *)&chinese_characters[p_datum][16], 16, 1000);
#if defined(__OLED12864_CS__)
	OLED12864_CS_H();
#endif
}
...
void Picture()
{
	...
	for(y=0;y<8;y++)
	{
		...
		for(x=0;x<132;x++) {Write_IIC_Data(show[i++]);}
    }
	OLED12864_CS_H();
}
*/
